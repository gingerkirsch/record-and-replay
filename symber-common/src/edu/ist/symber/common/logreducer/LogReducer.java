...
public class LogReducer {
	...
	/**
	 * Transforms a MapThreadToTrace (which uses long absolute values for
	 * clocks) into a MapThreadtoTraceShort (which uses short deltas instead of
	 * absolute values)
	 * 
	 * @param mapThreadtoTrace
	 * @return
	 */
	public static Map<String, ThreadTraceShort> reduceThreadTrace(
			Map<String, ThreadTrace> mapThreadtoTrace) {
		...
		for (Map.Entry<String, ThreadTrace> entry : mapThreadtoTrace.entrySet()) {
			...
			if (!trace.storeTrace.isEmpty()) {
				...
				newtrace.storeTrace.add(new Pair<Short, Short>(
						(short) (prevS.k - 0), (short) (prevS.v - 0))); // ** I
																		// subtract
																		// 0 to
																		// allow
																		// casting
																		...
																		// to
																		// short
				for (Pair<Long, Long> sclock : trace.storeTrace.subList(1,
						trace.storeTrace.size())) {
					Pair<Short, Short> newsclock = new Pair<Short, Short>(
							(short) 0, (short) 0);
			...
			if (!trace.loadTrace.isEmpty()) {
				...
				newtrace.loadTrace.add((short) (prevL - 0)); // ** I subtract 0
																// to allow
																// casting
																// easily to
																// short
				for (Long lclock : trace.loadTrace.subList(1,
						trace.loadTrace.size())) {
					short newlclock = 0;
			...
			if (!trace.syncTrace.isEmpty()) {
				...
				newtrace.syncTrace.add((short) (prevSync - 0)); // ** I subtract
																// 0 to allow
																// casting
																// easily to
																// short
				for (Long syncclock : trace.syncTrace.subList(1,
						trace.syncTrace.size())) {
					short newsclock = 0;
	...
	/**
	 * Transforms a MapThreadToTraceShort (which uses short deltas instead of
	 * absolute values) into a MapThreadtoTrace (which uses long absolute values
	 * for clocks)
	 * 
	 * @param mapThreadtoTrace
	 * @return
	 */
	public static Map<String, ThreadTrace> expandThreadTrace(
			Map<String, ThreadTraceShort> mapThreadtoTraceShort) {
		Map<String, ThreadTrace> res = new HashMap<String, ThreadTrace>();

		for (Map.Entry<String, ThreadTraceShort> entry : mapThreadtoTraceShort
				.entrySet()) {
			...
			if (!trace.storeTrace.isEmpty()) {
				Pair<Long, Long> prevS = new Pair<Long, Long>(
						(long) trace.storeTrace.firstElement().k,
						(long) trace.storeTrace.firstElement().v);
				newtrace.storeTrace.add(new Pair<Long, Long>(prevS.k, prevS.v));
				for (Pair<Short, Short> sclock : trace.storeTrace.subList(1,
						trace.storeTrace.size())) {
					Pair<Long, Long> newsclock = new Pair<Long, Long>((long) 0,
							(long) 0);
			...
			if (!trace.loadTrace.isEmpty()) {
				...
				newtrace.loadTrace.add((long) prevL); // ** I subtract 0 to
														// allow casting easily
														// to short
				for (Short lclock : trace.loadTrace.subList(1,
						trace.loadTrace.size())) {
					long newlclock = 0;
			...
			if (!trace.syncTrace.isEmpty()) {
				...
				newtrace.syncTrace.add((long) prevSync); // ** I subtract 0 to
															// allow casting
															// easily to short
				for (Short syncclock : trace.syncTrace.subList(1,
						trace.syncTrace.size())) {
					long newsclock = 0;
