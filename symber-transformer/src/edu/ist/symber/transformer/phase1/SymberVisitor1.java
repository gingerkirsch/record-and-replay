...
public class SymberVisitor1 extends Visitor {
	...
	public void visitInstanceFieldRef(SootMethod sm, Chain units, Stmt s,
		...
		if (context != RHSContextImpl.getInstance()) // ** CHANGE this may cut
														// off some shared vars
		{
			if ((/*
				 * Visitor.ftea.isFieldThreadShared(instanceFieldRef.getField())
				 * &&
				 */!Visitor.tlo.isObjectThreadLocal(instanceFieldRef, sm))
					|| sig.contains("TableDescriptor.referencedColumnMap")) // **
																			// TableDescriptor.referencedColumnMap
																			// is
																			// for
																			// derby2861
																			// bug
			{
				// if(!sig.contains("java.util.concurrent.locks")) //** we don't
				// want to trace locks
				sharedVariableWriteAccessSet.add(sig);
				// /////
				String shortclass = sm.getDeclaringClass().toString();
				if (shortclass.contains("$"))
					shortclass = shortclass.substring(0,
							shortclass.indexOf("$"));
				String tag;
				...
				String line = shortclass + "."
						+ instanceFieldRef.getField().getName() + "@" + tag;// +
																			// " instance";
				System.out
						.println("[SymbiosisTransformer] instance shared access: "
								+ line);
				// /////
			}
			// System.out.println("---> is field shared "+instanceFieldRef.getField()+"? "+Visitor.ftea.isFieldThreadShared(instanceFieldRef.getField()));
		}
	...
	public void visitStaticFieldRef(SootMethod sm, Chain units, Stmt s,
		...
		{
			if (/*
				 * Visitor.ftea.isFieldThreadShared(staticFieldRef.getField())
				 * &&
				 */!Visitor.tlo.isObjectThreadLocal(staticFieldRef, sm)) // **
																			// TableDescriptor.referencedColumnMap
																			// is
																			// for
																			// derby2861
																			// bug
			{
				...
				if (s.getTag("LineNumberTag") != null)
					...
					tag = "0";
				String line = shortclass + "."
						+ staticFieldRef.getField().getName() + "@" + tag;// +
																			// " instance";
				System.out
						.println("[SymbiosisTransformer] static shared access: "
								+ line);
				// /////
			}
