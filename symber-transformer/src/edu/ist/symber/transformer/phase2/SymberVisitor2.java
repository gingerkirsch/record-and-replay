...
public class SymberVisitor2 extends Visitor {
	...
	public void visitStmtEnterMonitor(SootMethod sm, Chain units, EnterMonitorStmt enterMonitorStmt) 
	{
		...
		Visitor.addCallAccessSyncObj(sm, units, enterMonitorStmt, "afterMonitorEnter",op, memory, "SYNCBLOCK");//*/
	...
	{
		String sigclass = invokeExpr.getMethod().getDeclaringClass().getName()+".OBJECT";
		...
		{
			Visitor.addCallAccessSyncLock(sm, units, s, "afterMonitorEnter",base, memory,"LOCK");
			Visitor.instrusharedaccessnum++;
			Visitor.sharedaccessnum++;
			Visitor.totalaccessnum++;
		} 
		else if(sig.equals("void await()") || sig.equals("void awaitNanos(long)"))	//** handles condition await
		{
			...
			while (stmtIt.hasNext()) 
			{
				...
				if(stmp instanceof AssignStmt && stmp.toString().contains(lockSig))
				{
			...
			Visitor.addCallAccessSyncAwait(sm, units, s, "afterConditionEnter", lock, base, memory, "AWAIT");
			Visitor.instrusharedaccessnum++;
			Visitor.sharedaccessnum++;
			Visitor.totalaccessnum++;
		...
		else if (sig.equals("void wait()")||sig.equals("void wait(long)") || sig.equals("void wait(long,int)")) //** handles monitor wait
		{       	
			Visitor.addCallAccessSyncWait(sm, units, s, "afterMonitorEnter", base, memory, "WAIT");
			Visitor.instrusharedaccessnum++;
			Visitor.sharedaccessnum++;
			Visitor.totalaccessnum++;
		...
		else if (sig.equals("void signal()") || sig.equals("void signalAll()")) //** handles condition signal
		{
			System.out.println("\ts : "+s);
			Value cond = ((AssignStmt)units.getPredOf(s)).getRightOp();
			String lockSig = Visitor.conditionToLockMap.get(cond.toString());
			...
			Visitor.addCallAccessSyncSignal(sm, units, s, "beforeConditionEnter", lock, base, memory,"SIGNAL");
			Visitor.instrusharedaccessnum++;
			Visitor.sharedaccessnum++;
			Visitor.totalaccessnum++;
		...
		else if (sig.equals("void notify()") || sig.equals("void notifyAll()")) //** handles monitor notify
		{
			Visitor.addCallAccessSyncNotify(sm, units, s, "beforeMonitorEnter",base, memory,"NOTIFY");
			Visitor.instrusharedaccessnum++;
			Visitor.sharedaccessnum++;
			Visitor.totalaccessnum++;
		...
		else if ((sig.equals("void join()") || sig.equals("void join(long)") || sig.equals("void join(long,int)")) && isThreadSubType(invokeExpr.getMethod().getDeclaringClass()))
		{
			Visitor.addCallJoinRunThreadAfter(sm, units, s, "joinRunThreadAfter", invokeExpr.getBase());
		}
		else if(invokeExpr.getMethod().isSynchronized() && Parameters.isReplay && !invokeExpr.getMethod().getSignature().startsWith("<java."))	//** used to instrument calls to synchronized methods in the caller method
		{	
	...
	public void visitStaticInvokeExpr(SootMethod sm, Chain units, Stmt s, StaticInvokeExpr invokeExpr, InvokeContext context) {
		...
		//** if we are dealing with accesses to sync methods from classes of Java libraries, we do not instrument them because soot does not analyze the JDK libraries (which could cause a deadlock)
		if(invokeExpr.getMethod().isSynchronized() && Parameters.isReplay && !invokeExpr.getMethod().getSignature().startsWith("<java."))	//** used to instrument calls to synchronized methods in the caller method
		{		
			String sigclass = invokeExpr.getMethod().getDeclaringClass().getName()+".OBJECT";
			Value memory = StringConstant.v(sigclass);
	...
	public void visitInstanceFieldRef(SootMethod sm, Chain units, Stmt s, InstanceFieldRef instanceFieldRef, RefContext context) 
	{
		...
		{
			if(Visitor.sharedVariableWriteAccessSet.contains(sig) && Parameters.traceSharedMem) //&& !Visitor.isWrappedByLock
			{		
				...
				Visitor.addCallAccessSPEInstance(sm,units, s, methodname, memory);
	...
	{      
		...
		{
			if(Visitor.sharedVariableWriteAccessSet.contains(sig) && Parameters.traceSharedMem) 
			{	
				//** handle loads and stores
				String methodname = "beforeLoad"; 
