...
public class XMHPAnalysis {
	...
	private void buildMHPLists() {
		...
		Map<Stmt, List<AllocNode>> startToAllocNodes = sjf
				.getStartToAllocNodes();
		Map<Stmt, List<SootMethod>> startToRunMethods = sjf
		...
		while (threadIt.hasNext()) {
			...
			// Get a list of all possible unique Runnable.run methods for this
			// thread start statement
			XAbstractRuntimeThread thread = new XAbstractRuntimeThread(); // provides
																			// a
																			// list
																			// interface
																			// to
			...
			// Find out if the "thread" in "thread.start()" could be more than
			// one object
			boolean mayStartMultipleThreadObjects = (threadAllocNodes.size() > 1);
			if (!mayStartMultipleThreadObjects) // if there's only one alloc
												// node
			{
				if (multiRunAllocNodes.contains(threadAllocNodes.iterator()
						.next())) // but it gets run more than once
				{
					mayStartMultipleThreadObjects = true; // then "thread" in
															// "thread.start()"
															// could be more
															// than one object
				}
