...
public class MyMonitorInfer {
	...
	public static void writingLog() {
		 ...
		 * file.createNewFile(); }else{ file.delete(); file.createNewFile(); }
		 * 
		 * FileWriter fw = new FileWriter(file.getAbsoluteFile(), true);
		 * BufferedWriter bw = new BufferedWriter(fw); Iterator<Integer> it=
		 * path.iterator(); while (it.hasNext()){ bw.write(it.next()+" "); }
		 * bw.close();
		 * 
